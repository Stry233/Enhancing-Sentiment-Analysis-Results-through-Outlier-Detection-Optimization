...
def start_test(train_data, test_data, model_name="microsoft/deberta-v3-small"):
    # Hyperparameters
    learning_rate = 2e-05
    train_batch_size = 16
    eval_batch_size = 16
    ...
    max_length = max(len(max(dataset['train']['text'], key=len)), len(max(dataset['test']['text'], key=len)))
    ...
    training_args = TrainingArguments(
        ...
        logging_steps=1,
        seed=seed,
        adam_beta1=optimizer_config["betas"][0],  # Using the beta1 value from optimizer_config
        adam_beta2=optimizer_config["betas"][1],  # Using the beta2 value from optimizer_config
        adam_epsilon=optimizer_config["epsilon"],  # Using the epsilon value from optimizer_config
        lr_scheduler_type="linear"
    )
...
def test_thresholds(filenames: list, test_data_path: str) -> tuple:
    """Tests each threshold and returns the logs, accuracies, and class accuracies."""
    logs = []
    accs = []
    class_accuracies = []
    for filename in filenames:
        log, acc, class_accuracy = start_test(filename, test_data_path)
        logs.append(log)
        accs.append(acc)
...
def plot_bars(ax, thresholds, values, title: str, ylabel: str):
    """Plots a bar chart for the given values."""
    bars = ax.bar(thresholds, values, width=0.05)
    for bar in bars:
        yval = bar.get_height()
        ax.text(bar.get_x() + bar.get_width() / 2, yval, round(yval * 100, 5), ha='center', va='bottom')
    ax.set_title(title)
...
if __name__ == "__main__":
    ...
    logs, accs, class_accuracies = test_thresholds(filenames, test_file_location)
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig(f"../imgs/analyze/{dataset}_analyze.png")
