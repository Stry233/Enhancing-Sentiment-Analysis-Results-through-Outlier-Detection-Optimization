...
from transformers import AutoModelForSequenceClassification, AutoTokenizer, Trainer, TrainingArguments, \
    IntervalStrategy, T5Tokenizer, T5ForConditionalGeneration
...
def start_test(train_data, test_data, model_name="distilbert-base-uncased"):
    ...
    max_length = 512 # max(len(max(dataset['train']['text'], key=len)), len(max(dataset['test']['text'], key=len)))
...
if __name__ == "__main__":
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig(f"./result/{dataset}_analyze.png")
