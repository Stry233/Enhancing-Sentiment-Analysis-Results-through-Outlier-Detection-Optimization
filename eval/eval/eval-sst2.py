...
def start_test(train_data, test_data, model_name="t5-3b"):
    # Hyperparameters
    learning_rate = 2e-05
    train_batch_size = 16
    eval_batch_size = 16
    ...
    # Tokenize the data
    # Note: The text is now a combination of input text and label text for T5's text-to-text format.
    def tokenize(example_batch):
        # Tokenize the text
        text = example_batch['text']
        text = "classify: " + text
        text_tokenized = tokenizer(text, truncation=True, padding='max_length', max_length=512)

        # Tokenize the label
        label = example_batch['label']
        label_name = "positive" if label == 1 else "negative"
        label_tokenized = tokenizer(label_name, return_tensors="pt").input_ids[0]
...
if __name__ == "__main__":
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig(f"../imgs/analyze/{dataset}_t5-3b_analyze.png")
